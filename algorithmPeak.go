		...
		if input.resetPeriod > 0 {
			...
			if pv.resetStart+int64(input.resetPeriod) < _nowTime {
				if pv.maxValue != 0 {
					...
					pv.maxPerValue, pv.maxPerTime = timePeakExtraction(pv.maxValue, _nowTime, pv.maxPerValue, pv.maxPerTime, _tagName, date)
				}
...
func timePeakExtraction(maxValue float64, _nowTime int64, maxPerValue [4]float64, maxPerTime [4]int64, _tagName string, date string) ([4]float64, [4]int64) {
	...
	for i := 0; i < len(divTime); i++ {
		...
		if maxPerTime[i] > _nowTime || maxPerTime[i] == 0 {
			...
			strExtaction := fmt.Sprintf("Peak/%s/%f/%s/%s", t2, maxPerValue[i], _tagName, divTimeStr[i])
			outputExtraction(strExtaction)
			maxPerValue[i] = 0
			maxPerTime[i] = 0
